...
#
# For each problem, we've got a couple of excellent posts explaining how to approach it. However, most of the posts failed ' \
#                     'to identify the connections among these problems and made it hard to develop a consistent way of ' \
#                     'dealing with this series of problems. Here I will introduce the most generalized solution applicable ' \
#                     'to all of these problems, and its specialization to each of the six problems above.
...
class Solution121:
    def maxProfit(self, prices: List[int]) -> int:
        p_k0 = 0  # profit, at time i, k transactions, and 0, 1 stock
        p_k1 = -inf

        for price in prices:
            p_k0 = max(p_k0, p_k1 + price)
            p_k1 = max(p_k1, -price)

        return p_k0


# Now let's try to gain some insight of the solution above. If we examine the part inside the loop more carefully, T_i11
# really just represents the maximum value of the negative of all stock prices up to the i-th day, or equivalently the minimum
# value of all the stock prices. As for T_i10, we just need to decide which action yields a higher profit, sell or rest. And if
# action sell is taken, the price at which we bought the stock is T_i11, i.e., the minimum value before the i-th day. This is exactly
# what we would do in reality if we want to gain maximum profit. I should point out that this is not the only way of solving
# the problem for this case. You may find some other nice solutions here.
...
#
# This solution suggests a greedy strategy of gaining maximum profit: as long as possible, buy stock at each local
# minimum and sell at the immediately followed local maximum. This is equivalent to finding increasing subarrays in prices
# (the stock price array), and buying at the beginning price of each subarray while selling at its end price. It's easy
# to show that this is the same as accumulating profits as long as it is profitable to do so, as demonstrated in this post.
...
class Solution123:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        for price in prices:
            p_20 = max(p_20, p_21 + price)
            p_21 = max(p_21, p_10 - price)

            p_10 = max(p_10, p_11 + price)
            p_11 = max(p_11, -price)
...
# to 0 or 1 stocks in hand at the end of the day. However, there is a minor optimization we can do if k exceeds some critical
# value, beyond which the maximum profit will no long depend on the number of allowable transactions but instead will be bound
# by the number of available stocks (length of the prices array). Let's figure out what this critical value will be.

# A profitable transaction takes at least two days (buy at one day and sell at the other, provided the buying price is less
# than the selling price). If the length of the prices array is n, the maximum number of profitable transactions is n/2
# (integer division). After that no profitable transaction is possible, which implies the maximum profit will stay the same.
# Therefore the critical value of k is n/2. If the given k is no less than this value, i.e., k >= n/2, we can extend k to
# positive infinity and the problem is equivalent to Case II.
#
...
#     return T_ik0[k];
# }
# The solution is similar to the one found in this post. Here I used backward looping for the T array to avoid using
    ...
    def maxProfit(self, k: int, prices: List[int]) -> int:
        ...
        for price in prices:
            for i in range(k, 0, -1):
                p_k0[i] = max(p_k0[i], p_k1[i] + price)
                p_k1[i] = max(p_k1[i], p_k0[i - 1] - price)

        return p_k0[-1]

    def freeTrade(self, prices):
...
#
# This case resembles Case II very much due to the fact that they have the same k value, except now the recurrence
...
#
# But with "cooldown", we cannot buy on the i-th day if a stock is sold on the (i-1)-th day. Therefore, in the second
# equation above, instead of T[i-1][k][0], we should actually use T[i-2][k][0] if we intend to buy on the i-th day.
...
class Solution309:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        for price in prices:
            p_k0_old = p_k0
            p_k0 = max(p_k0, p_k1 + price)
            p_k1 = max(p_k1, p_kCooldown - price)
            p_kCooldown = p_k0_old
...
#
# Again this case resembles Case II very much as they have the same k value, except now the recurrence relations need to
# be modified slightly to account for the "transaction fee" requirement. The original recurrence relations for Case II are given by
#
# T[i][k][0] = max(T[i-1][k][0], T[i-1][k][1] + prices[i])
# T[i][k][1] = max(T[i-1][k][1], T[i-1][k][0] - prices[i])
#
# Since now we need to pay some fee (denoted as fee) for each transaction made, the profit after buying or selling the
# stock on the i-th day should be subtracted by this amount, therefore the new recurrence relations will be either
#
# T[i][k][0] = max(T[i-1][k][0], T[i-1][k][1] + prices[i])
...
# T[i][k][1] = max(T[i-1][k][1], T[i-1][k][0] - prices[i])
#
# Note we have two options as for when to subtract the fee. This is because (as I mentioned above) each transaction is
# characterized by two actions coming as a pair - - buy and sell. The fee can be paid either when we buy the stock
# (corresponds to the first set of equations) or when we sell it (corresponds to the second set of equations). The
# following are the O(n) time and O(1) space solutions corresponding to these two options, where for the second solution
...
class Solution714:
    def maxProfit(self, prices: List[int], fee: int) -> int:
        ...
        for price in prices:
            p_k0_old = p_k0
            p_k0 = max(p_k0, p_k1 + price)
            p_k1 = max(p_k1, p_k0_old - price - fee)
