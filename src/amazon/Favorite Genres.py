...
import collections
def find(userSongs,songGenres):

    song_to_genres={}

    for key,vals in songGenres.items():
        for val in vals:
            song_to_genres[val] = key


    song_genres_num = collections.defaultdict(dict)

    for key, vals in userSongs.items():

        for val in vals:
            genre = song_to_genres[val]
            if genre not in song_genres_num[key]:
                song_genres_num[key][genre]=1
            else:
                song_genres_num[key][genre]+=1
    ...
    for key, vals in song_genres_num.items():
        m = float('-inf')
        for genre, count in vals.items():
            if count <m:
                continue
            if count > m:
                m = count
