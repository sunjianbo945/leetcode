...
class Solution:
    def fiveStarReviews(self, productRatings: List[List[int]], ratingsThreshold: int) -> int:
        total_product = len(productRatings)
        total_rating_percentage = 0
        heap = []
        for rating, count in productRatings:
            precentage = rating / count
            total_rating_percentage += rating / count
            heappush(heap, (-((rating + 1) / (count + 1) - precentage), rating, count))

        res = 0
        while total_rating_percentage / total_product < ratingsThreshold / 100:
            _, rating, count = heappop(heap)
            total_rating_percentage -= rating / count
            # five star
            count += 1
            rating += 1
            percentage = rating / count
            heappush(heap, (-((rating + 1) / (count + 1) - percentage), rating, count))
            total_rating_percentage += percentage
            res += 1
